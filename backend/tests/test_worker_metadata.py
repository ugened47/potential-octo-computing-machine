...
async def test_extract_video_metadata_extracts_duration_resolution_format(
    ...
    with patch("arq.create_pool"), patch("arq.connections.RedisSettings"):
        ...
        with patch("app.services.video_metadata.VideoMetadataService.download_video_from_s3"), \
             patch("app.services.video_metadata.VideoMetadataService.extract_metadata") as mock_extract:
...
@pytest.mark.asyncio
async def test_extract_video_metadata_status_updates_flow(
    db_session, test_user: User
):
...
async def test_extract_video_metadata_error_handling_sets_failed_status(
    ...
    with patch("arq.create_pool"), patch("arq.connections.RedisSettings"):
        ...
        with patch("app.services.video_metadata.VideoMetadataService.download_video_from_s3") as mock_download:
...
@pytest.mark.asyncio
async def test_extract_video_metadata_redis_progress_tracking(
    db_session, test_user: User
):
